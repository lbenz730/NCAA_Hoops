...
tourney_sim_single  <- function(teams, seeds, byes, double_byes, hca, eliminated, return_list = F) {
  champ <-
    tryCatch({
      ### Champ Storage
      n <- length(teams)
      ...
      if(byes > 0) {
        ...
        games$pred_score_diff[cur_round] <- predict(lm.hoops, newdata = games[cur_round,])
        games$win_prob[cur_round] <- predict(glm.pointspread, newdata = games[cur_round,] , type = "response")
        games$win_prob[cur_round] <- case_when(eliminated[games$team_seed[cur_round]] ~ 0,
                                               eliminated[games$opp_seed[cur_round]] ~ 1,
                                               T ~ games$win_prob[cur_round])
        games$winner[cur_round] <- ifelse(games$win_prob[cur_round] >= runif(length(cur_round)), games$team[cur_round],
                                          games$opponent[cur_round])
        games$opponent[next_round[length(cur_round):1]] <- games$winner[cur_round] 
        games$opp_seed[next_round[length(cur_round):1]] <- 
      ...
      if(!return_list) {
        ...
        finals <- c(games$team[n-1], games$opponent[n-1])
        loser <- setdiff(finals, games$winner[n-1])
        c(games$winner[n-1], loser)
      }
    }, error = function(e) {
      NA 
    })
